...
- **Интеллектуальный помощник:** Предоставление консультаций и помощи по работе с порталом [zakupki.mos.ru](https://zakupki.mos.ru).
- **Обработка запросов с помощью ИИ:** Использование локальной нейросетевой модели для понимания и ответов на вопросы пользователей.
- **Взаимодействие через Телеграм-бот:** Удобный интерфейс для общения с ассистентом.
- **Управление учетными записями:** Регистрация и аутентификация пользователей.
- **История чатов:** Сохранение и доступ к истории переписки.
- **Эскалация на оператора:** Возможность запросить помощь живого оператора.
- **Безопасность:** Реализованы механизмы защиты, включая CSRF и безопасное хранение токенов.
- **Подробное описание API и команд бота** см. в соответствующих разделах ниже.
...
Он представляет собой значимый шаг в автоматизации взаимодействия с государственными закупочными порталами. Его уникальность заключается в комбинации локально развертываемой ИИ-модели, обеспечивающей приватность и скорость, с удобным интерфейсом Телеграм-бота. Это позволяет снизить порог входа для пользователей портала zakupki.mos.ru, ускорить поиск информации и получение поддержки.

### Дальнейшие улучшения
Возможные направления для развития проекта:
- **Расширение базы знаний ИИ:** Интеграция с большим объемом нормативных документов и инструкций.
- **Проактивные уведомления:** Информирование пользователей о новых тендерах или изменениях в законодательстве.
- **Голосовой интерфейс:** Возможность взаимодействия с ассистентом голосом.
- **Расширенная аналитика:** Предоставление пользователям статистики по их запросам и взаимодействиям.
- **Интеграция с другими сервисами:** Например, с календарями для напоминаний о сроках подачи заявок.
